...
# Properties that can be configured in the inspector
@export var channel_to_display: ChannelType = ChannelType.LEFT
...
func _process(_delta: float) -> void:
	...
	for i in range(1, VU_COUNT + 1):
		...
		# Get the amplitude for the specified channel
		var magnitude: float
		if channel_to_display == ChannelType.LEFT:
			magnitude = spectrum.get_magnitude_for_frequency_range(prev_hz, hz).x
		else:  # RIGHT
			magnitude = spectrum.get_magnitude_for_frequency_range(prev_hz, hz).y
	...
	# Since sound is continuously played, the graph needs to be updated every frame
	queue_redraw()
